...
function PANEL:Init()
    ...
    x3m4kDirPanel:SetSize(0, 0)
    x3m4kDirPanel:SetPos(100, 395)
...
local spray_key_name = string.upper(language.GetPhrase(input.GetKeyName(bind2key("impulse 201")) or ""))
...
function PANEL:Paint(w, h)
    ...
    surface.SetDrawColor(0, 0, 0, 200)
    surface.DrawRect(25, 105, 400, 195)
    ...
    surface.SetTextPos(30, getDrawX(drawX))
    surface.DrawText("[" .. spray_key_name .. "] " .. language.GetPhrase("#mappatcher.hud.teleport_to_active_object"))
    surface.SetTextPos(30, getDrawX(drawX))
    ...
    if MapPatcher.Editor.Object and MapPatcher.Editor.Object.ID ~= 0 then
        ...
        local activeObjLen = #language.GetPhrase("#mappatcher.hud.active_object")

        surface.DrawText("#mappatcher.hud.active_object")
        surface.DrawText(" " .. (tostring(MapPatcher.Editor.Object)):sub(1, 70 - activeObjLen))
    else
        surface.SetDrawColor(MapPatcher.Editor.Object.TextureColor)
        surface.DrawRect(25, drawX[1] + 40 + 30 + 10, 400, 30)
        surface.SetTextPos(30, drawX[1] + 87)
...
function PANEL:OnKeyCodePressed(key_code)
    ...
    elseif key_code == bind2key("impulse 201") then
        if MapPatcher.Editor.Object ~= nil and MapPatcher.Editor.Object.ID ~= 0 then
            MapPatcher.Editor.SelectObject(MapPatcher.Editor.Object, true)
        end
    end
end
...
function PANEL:Think()
    ...
    if MapPatcher.Editor.Tool == "x3m4k_push" then
        self.x3m4kDirections:GetParent():SetSize(200, 200)
        self.x3m4kDirections:SetSize(200, 200)
    else
        self.x3m4kDirections:GetParent():SetSize(0, 0)
        self.x3m4kDirections:SetSize(0, 0)
    end
