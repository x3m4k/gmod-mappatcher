...
util.AddNetworkString("mappatcher_focus")

MapPatcher.Brushes = MapPatcher.Brushes or {}
MapPatcher.TempDisallowedPickups = {}
...
function MapPatcher.DisallowPlayersPickup(time, ent)
  if MapPatcher.TempDisallowedPickups[ent:GetCreationID()] ~= nil then
    return
  end
  ...
  MapPatcher.TempDisallowedPickups[ent:GetCreationID()] = true

  -- infinite disallow (until a map load)
  if time == -1 then
    return
  end
  ...
  timer.Simple(time, function()
    ...
    MapPatcher.TempDisallowedPickups[ent:GetCreationID()] = nil
  end)
end

function MapPatcher.NetworkObjects(objects, ply)
  if not istable(objects) then
    error("First argument must be table of objects")
    return
  end
...
net.Receive("mappatcher_submit", function(len, ply)
  ...
  local focus_type = net.ReadUInt(2)
  ...
  if focus_type ~= 0 then
    net.Start("mappatcher_focus")
    net.WriteUInt(object.ID, 16)
    net.WriteBool(focus_type == 2)
    net.Send(ply)
  end
end)

hook.Add("PlayerInitialSpawn", "MapPatcher", function(ply)
  if #MapPatcher.Objects == 0 then
    return
  end
...
-- block unwanted actions on MapPatcherObjects (Context menu->Remove object and etc.)
hook.Add("CanProperty", "MapPatcher", function(ply, property, ent)
  if ent.MapPatcherObject then
    return false
  end
end)

-- block pickups after being dropped by MapPatcher brushes.
hook.Add("AllowPlayerPickup", "MapPatcher", function(ply, ent)
  if MapPatcher.TempDisallowedPickups[ent:GetCreationID()] == true and not ply:IsAdmin() then
    return false
  end
end)
hook.Add("GravGunPickupAllowed", "MapPatcher", function(ply, ent)
  if MapPatcher.TempDisallowedPickups[ent:GetCreationID()] == true and not ply:IsAdmin() then
    return false
  end
end)
hook.Add("PhysgunPickup", "MapPatcher", function(ply, ent)
  if MapPatcher.TempDisallowedPickups[ent:GetCreationID()] == true and not ply:IsAdmin() then
    return false
  end
end)
-- block pickups ends here
