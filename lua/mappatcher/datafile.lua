...
local data_block_readers = {
  [DATABLOCK_MESH_PLAYERCLIP] = function(buffer, len)
    basic_mesh_reader(buffer, len, "playerclip")
  end,
  [DATABLOCK_MESH_KILL] = function(buffer, len)
    basic_mesh_reader(buffer, len, "kill")
  end,
  [DATABLOCK_MESH_BULLETCLIP] = function(buffer, len)
    basic_mesh_reader(buffer, len, "bulletclip")
  end,
  [DATABLOCK_MESH_PROPCLIP] = function(buffer, len)
    basic_mesh_reader(buffer, len, "propclip")
  end,
  [DATABLOCK_MESH_CLIP] = function(buffer, len)
    basic_mesh_reader(buffer, len, "clip")
  end,
  [DATABLOCK_TOOL_OBJECT] = function(buffer, len)
    ...
    if object and object:ReadFromBuffer(buffer, data_len, version) ~= false then
      ...
      if data_len ~= (data_end - data_start) then
        ErrorNoHalt(
          "[MapPatcher] Object data read incorrectly (class:"
          .. class_name
          .. ")(data_len:"
          .. data_len
          .. ")(read:"
    ...
    elseif object then
      ErrorNoHalt(
        "[MapPatcher] Could not read object data (class:"
        .. class_name
        .. ")(object_version:"
        .. version
        .. ")(current_version:"
...
function MapPatcher.LoadObjectsFromFile()
  ...
  local block_data_end
  repeat
    if buffer:Tell() >= buffer:Size() - 1 then
      break
    end
    ...
    if block_len ~= (block_end - block_start) then
      ErrorNoHalt(
        "[MapPatcher] Block data read incorrectly (type:"
        .. block_type
        .. ")(data_len:"
        .. block_len
        .. ")(read:"
